...
use totex_host::{space, wsl};

use super::{Door, LOOPBACK};
...
pub fn dressing(door: &Door, id: &str, cwd: &str) -> Vec<(String, String)> {
    let Some((url, token)) = address(door, id, cwd) else {
        return Vec::new();
    };
    vec![
...
fn address(door: &Door, id: &str, cwd: &str) -> Option<(String, String)> {
    ...
    let (port, keys) = {
        let standing = door.standing();
        (standing.as_ref()?.port, door.keys.clone())
    };
    ...
    if !space::settings(Path::new(cwd)).mcp {
        return None;
    }
...
pub(super) fn shares_loopback(distro: &str) -> bool {
    ...
    if let Some(answer) = lock(known).get(distro) {
        return *answer;
    }
    ...
    let answer = wsl::exec(distro, None, &[], &["wslinfo", "--networking-mode"])
        ...
        .unwrap_or(false);
    lock(known).insert(distro.to_string(), answer);
    answer
}
