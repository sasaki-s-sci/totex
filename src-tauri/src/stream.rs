//! Carrying what a child process says to the window.
//!
...
//! it arrives is what makes output stutter — every message crosses to the
//! window as a script for it to evaluate, on the same thread that draws — so
//! the cost is paid per character rather than per screenful.
//!
//! Reading and handing over are therefore separated. The reader never waits on
//! the window, and what it reads is gathered for a fraction of a frame before
    ...
    F: Fn(String) -> bool + Send + 'static,
{
    ...
    // on the window: neither is allowed to hold up the other.
    thread::spawn(move || {
...
mod tests {
    ...
    #[test]
    fn a_window_that_has_gone_away_ends_the_read() {
