//! What crosses out of this module: to the window, and to whatever is following
//! the sessions.
...
/// What the sessions do, for whatever is following them — the things a program
/// on the other end of a socket would have to be handed, and nothing else.
#[derive(Clone, Copy)]
pub enum Event<'a> {
...
/// once per session as that session is about to run. Registered for the same
/// reason a follower is: what goes in an environment besides the terminal's own
/// name is a fact about the rest of the app.
